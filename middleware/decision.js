var conf = require('./../config/convict.js'),
    ...
    quakeRoot = 'http://' + conf.get('quake_host') + ':' + conf.get('quake_port'),
    quiverRoot = 'https://' + conf.get('quiver_host') + ':' + conf.get('quiver_port');

app.get('/quake-sdk/auth/authorize', function (req, res) {
    var params = JSON.parse(res.text);
  console.log('decision', req, res);
  request.get(quakeRoot + '/auth/decision/authorization')
  res.end();
});
